...
A single-planet roguelike run: cinematic → draft → 3 floors (3+ waves each) → boss → win/lose. Desktop only, Windows first. Target play time: 25-35 min per run. Replayable via 3 role drafts.
...
| 3 | 3 floors per planet with escalating waves | `DONE` | Gateway → Conduit → Arena, 3+ waves each |
| 4 | Boss on final floor | `DONE` | Floor 3 boss wave |
...
| # | Feature | Status | Notes |
|---|---------|--------|-------|
| 10 | 3 map layouts with terrain features | `DONE` | Gateway/Conduit/Arena with elevated, channels, datastreams |
...
| 13 | Sound design (placeholder minimum) | `TODO` | Signal fire, gate open, turret shot, enemy death |
| 14 | 3-4 corruption/modifier events | `TODO` | AXIS possession, signal jam, overloader |
| 15 | Larger battlefield for enemy variety | `TODO` | Current maps too small for off-screen spawning |
...
| 22 | Fog/atmosphere | `WIP` | Tron fog banks exist, some visual bugs |
...
1. **Sound design** — Procedural PCM sounds for signal fire, turret shot, enemy death, wave start/end
2. **Corruption events** — AXIS possession mid-wave, signal jammer, overloader
3. **Larger battlefield option** — Needed for Planet 2 mercenary spawn design

### Tier 2: Polish
4. **Visual juice** — Death pops, screen shake on boss hits
5. **Fog cleanup** — Tron fog quality from bug reports
6. **Final balance pass** — Wave difficulty, signal power values, gold economy
