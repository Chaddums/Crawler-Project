...
## Current Code vs. Design Terminology Gap

**`VineSpawnGroup` / `Groups` in code = Surge in design.**  
This rename has not been applied to the codebase yet. When touching wave-related code, rename on sight:
- `VineSpawnGroup` → `SurgeData` (or `VineSurgeData`)
- `Groups` list on `VineWaveData` → `Surges`

**`Gold` / `CurrentScrap` in GameManager = placeholder economy.** The full resource system (see below) has not been implemented. Do not build new features on top of the current Gold system.

---

## Resource System (In Design — Partially Implemented)

The game has two resources. These replace the current Gold placeholder entirely.
...
Currently all waves use implicit `KillAll` — wave ends when all spawned enemies are dead.

**Planned tunable `completionMode` on Wave (not yet implemented):**
```
...
## Spawner / Scaler Architecture (Planned, Not Implemented)

Surges own base spawn definitions. Waves can override/modify surge definitions. Floors apply multipliers on top. Each layer scales independently.

All tuning values must be **data-driven (JSON)**. Designers must be able to change values without code changes.
...
## Commander System (Greenfield — Not Yet Implemented)
...
- **`VineSpawnGroup` / `Groups` = Surge.** Rename on sight.
...
- **Do not build on top of the Gold/currency system.** It is deprecated. Scrap + Magic replaces it entirely.

---

## Outstanding TBDs

- Mining Building: passive generation rates for Scrap mode vs. Magic mode
- Magic accumulation meter: visual design, discrete thresholds vs. smooth scaling
- Magic shop: specific upgrade options per floor per planet for all three magic types
- Character names and full ability kits for all three characters
- Assassin `onReachConsequence`: type and effect values not decided
- Commander `telegraphDuration` exact value (~2–3s range)
- Full list of supported `Reactive` Commander trigger types
- Rally behavior specifics
- JSON migration from hardcoded `VineWaveRegistry`
- Surge-level Commander integration into `VineWaveData`
- Tunable `completionMode` implementation on Wave
- Planet names for Planets 2 and 3 (Planet 1 = Grid Prime)
