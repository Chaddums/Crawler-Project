...
- **Dome clipping fixes** — floor disc, drill, ring boundary
- **BIT color fixes** — corruption event color revert, magic color matching

### Claude B — Data Architecture/Systems
...
All gameplay bugs resolved and archived. Remaining minor items (post-alpha):
- **DifficultyScaler not wired** — registered but only spawn accumulator reads surge multiplier
- **EntityRegistry empty** — registered but towers/enemies don't register/unregister yet
...
## Not Yet Implemented (Post-Alpha)
...
- **More enemy types** — models exist for wire_worm, volt_sprinter, overclock_drone, etc.
