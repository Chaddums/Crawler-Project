...
namespace JunkbotArena.Editor
{
    ...
    public partial class AudioClipEditor : VBoxContainer
    {
        ...
        private bool ParseWavFile(string resPath)
        {
            ...
            // Convert to mono short[] for display/editing
            ConvertToSamples();
            return true;
        }
        ...
        private void ConvertToSamples()
        {
            ...
            {
                // 24/32-bit: just read as 16-bit truncated (rare for game SFX)
        ...
        private void Normalize()
        {
            ...
            // Calculate gain needed to reach 0dBFS (32767)
        ...
        private static bool WriteWav(string path, short[] samples, int sampleRate)
        {
            ...
            file.Store32(16);                        // chunk size
            file.Store16(1);                         // PCM format
            file.Store16(1);                         // mono
            file.Store32((uint)sampleRate);          // sample rate
            file.Store32((uint)(sampleRate * 2));    // byte rate (mono 16-bit)
        ...
        private partial class WaveformView : Control
        {
            ...
            public override void _Draw()
            {
                ...
                // Trim region overlay (dim outside)
                ...
                if (Samples.Length > 0)
                {
                    float durSec = Samples.Length / 22050f; // approximate for label
                    // We don't know sampleRate here, so just show ratio
                }
            }
